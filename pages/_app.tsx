...
const { Terminal, consoleFunction } = (() => {
  ...
  function Terminal(props: { exitCommand?: () => void, powerSaveingMode: boolean }) {
        ...
        },
        {
          ...
          async action(param) {
            const shellMsgs = [
              ...
              <>{"Version // 0.3.0"}</>,
              emptyLine,
              <>{"這個就是一個很智障的東西 難用死 拓展性差"}</>,
              <>{"但真的很好玩 就....特別好玩"}</>,
              <>{"反正 寫這個東西的目的 就是 帥 就這樣 沒了"}</>,
              emptyLine,
              <>{"小故事 : "}</>,
              <>{"之所以叫 \"奇妙打指令的地方\" 是因爲之前"}</>,
              <>{"在AD還有管理員的時候 他在我伺服器建了一個用機器人的頻道"}</>,
              <>{"頻道名稱就真的叫做 \"奇妙打指令的地方\""}</>,
              <>{"後來寫了這個東西 在想中文名要叫啥 然後 想到這個東西 就這樣叫了"}</>,
              emptyLine,
              <>{"#####################################"}</>,
            ]

            if (param.type) {
              const type = param.type as ("shell" | "project" | "fontuse" | "iconuse" | "music&sfx" | "kilo")
              switch (type) {
                ...
                case 'fontuse': {
                  ...
                  const msgs = [
                    ...
                    <>{"[ 字體的使用 ]"}</>,
                    emptyLine,
                    <>{"# "}<a href='https://store.justfont.com/' target='_blank'>{"JustFont"}</a>{" - "}<a href='https://justfont.com/huninn/' target='_blank'>{"OpenHuninn 2.1"}</a></>,
                    <>{"一個十分好的這個 很圓的字體"}</>,
                    <>{"目前所有的排版都是針對這個字體下去設計的"}</>,
                    <>{"所以換了字體 我的網頁就會直接炸掉"}</>,
                    emptyLine,
                    <>{"OpenHuninn : "}<a href='https://justfont.com/huninn/' target='_blank'>{"https://justfont.com/huninn/"}</a></>,
                    <>{"JustFont : "}<a href='https://store.justfont.com/' target='_blank'>{"https://store.justfont.com/"}</a></>,
                    emptyLine,
                    <>{"# "}<a href='https://github.com/ryanoasis/nerd-fonts/releases/download/v3.4.0/Hack.zip' target='_blank'>{"Hack Nerd Font"}</a></>,
                    <>{"給終端 程式碼用的"}</>,
                    <>{"會直接替代monospace 啊 十分的好"}</>,
                    <>{"所以你在那些 額 什麽鳥地方看到的那些 帶圖示的字符"}</>,
                    <>{"在這個終端裏面 理論上來講 應該都是可以用的"}</>,
                    emptyLine,
                    <>{"然後他的字體樣式有一堆 啊 一個一個慢慢做適配 啊去世"}</>,
                    <>{"不過適配完了基本就沒事了"}</>,
                    emptyLine,
                    <>{"Nerd Fonts : "}<a href='https://www.nerdfonts.com/' target='_blank'>{"https://www.nerdfonts.com/"}</a></>,
                    emptyLine,
                    <>{"#####################################"}</>,
                  ]
                ...
                case 'iconuse': {
                  ...
                  const msgs = [
                    ...
                    <>{"[ 圖示的使用 ]"}</>,
                    emptyLine,
                    <>{"# "}<a href='https://fonts.google.com/icons' target='_blank'>{"Google.Font"}</a></>,
                    <>{"大部分常見的圖示都是這裏抓來的"}</>,
                    emptyLine,
                    <>{"Font.Google : "}<a href='https://fonts.google.com/icons' target='_blank'>{"https://fonts.google.com/icons"}</a></>,
                    emptyLine,
                    <>{"# "}<a href='https://icons8.com/' target='_blank'>{"Icons8"}</a></>,
                    <>{"然後像是應用程式的圖示啊 之類的 啊 隨便啦"}</>,
                    emptyLine,
                    <>{"Icons8 : "}<a href='https://icons8.com/' target='_blank'>{"https://icons8.com/"}</a></>,
                    emptyLine,
                    <>{"#####################################"}</>,
                  ]
                ...
                case 'music&sfx': {
                  ...
                  const msgs = [
                    ...
                    <>{"音樂我寫的"}</>,
                    <>{"音效也是"}</>,
                    <>{"拜托看一下我 "}<a href='https://open.spotify.com/artist/1MCtBV218RV96NvuvOtPEN' target='_blank'>{"spotify"}</a>{" 和 "}<a href='https://music.youtube.com/channel/UCI9Fxq7m3MupJPziIzFJqCg' target='_blank'>{"youtube music"}</a></>,
                    <>{"額 雖然 好像....好對不起 好像沒什麽好看的算了"}</>,
                    <>{"反正 這邊先甩兩個鏈接"}</>,
                    emptyLine,
                    <>{"spotify : "}<a href='https://open.spotify.com/artist/1MCtBV218RV96NvuvOtPEN' target='_blank'>{"https://open.spotify.com/artist/1MCtBV218RV96NvuvOtPEN"}</a></>,
                    <>{"youtube music : "}<a href='https://music.youtube.com/channel/UCI9Fxq7m3MupJPziIzFJqCg' target='_blank'>{"https://music.youtube.com/channel/UCI9Fxq7m3MupJPziIzFJqCg"}</a></>,
                    emptyLine,
                    <>{"#####################################"}</>,
                  ]
        ...
        },
        {
          ...
          action: async () => {
            ...
            const rootElements = [
              ...
              "InputOverlay",
              "Main",
            ].map(id => document.getElementById(style[id])!)

            for (const element of rootElements) {
              async function remove(ele: HTMLElement) {
                const children = Array.from(ele.children);

                logWithoutTimeout(<div style={{ color: "red" }}>{`${ele.tagName}${ele.id ? "#" + ele.id : ""}${ele.classList ? "." + ele.classList.value.split(" ").join(".") : ""} [ removed ]`}</div>);
...

            {
              const owo = document.getElementById(style["Console"])!.style
              owo.height = "100%"
              owo.transform = " translate(-50%, 0)"
              owo.transition = "none"
            }

            setMessage([])

            await runCommand("color set #ff0000")
    ...
    const glabsList: CommandType[] = useMemo(() => [
      {
        name: "help",
        dsc: "List All Commands",
        alias: ["?"],
      ...
      },
      {
        ...
        async action() {
          document.getElementById(style["Console"])!.remove();
    ...
    useEffect(() => {
      ...
      const allCmds = [...(!glabs ? defaultCommandList : glabsList), ...customCmds];
    ...
    }, [customCommand, defaultCommandList, glabsList, log, glabs]);
...
function MyApp({ Component, pageProps }: AppProps) {
  ...
  const [NONE_BG, SET_NONE_BG] = useState<boolean>(false)
  ...
  useEffect(() => {
    ...
    document.getElementsByTagName("html")![0].style.background = NONE_BG ? "none" : ""
